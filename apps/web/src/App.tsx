...
export default function App() {
  ...
  function comingSoon() {
    setNotice("Rooms land in Sprint 2 — the engine is being built!");
  }

  function handleJoin() {
    const code = joinCode.trim().toUpperCase();
    if (!isValidRoomCode(code)) {
      setNotice("Room codes are 6 characters, like AB3Xx9");
      return;
    }
    comingSoon();
  }
        ...
        <main className="flex flex-1 items-center px-6 sm:px-10">
          <div className="max-w-xl">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              <button
                onClick={comingSoon}
                className="rounded-xl bg-accent px-7 py-3.5 font-semibold text-bg transition hover:brightness-110 active:scale-[0.98]"
              >
                Create a Room
              </button>
